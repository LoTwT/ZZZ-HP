...
| **要点** | 招式 = 伤害类型 ×1 + 招式类型 ×N + 增益锚点 ×0..1；异常类两者均留空。公共预设仅按元素的属性异常（含霜、无流明）。招式库筛选：直伤类/异常类、自建库/预设库；公共异常只出本元素 |
...
| **状态** | **骨架阶段**，未发版。计算页页内展开招式流程；公共预设仅按元素的属性异常。待办：工作区 `dev-docs/skill-flow-todo.md` |
