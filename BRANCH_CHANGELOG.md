...
**当前推荐发布线**：`3.1.6`（已合入 `3.1.6.3` 尖端：handoff1 + 招式流程 / 方案事件 + 计算器 UI）。  
**当前开发线**：`3.1.6.3`（继续迭代；**保留**本分支；已含 `3.1.6.4` 招式流程线，**保留** `3.1.6.4`）。  
**上一完整线**：`3.1.6.1` / `3.1.6.2`（尖端在合入前相同）。  
**稳定主干**：`main`（含早期 3.1.6 级管理端鉴权；完整体验请用发布线 `3.1.6`）。
...
| **package** | `3.1.6.3` |
| **分支关系** | **已并回** `3.1.6`（发布线）；**本分支保留**继续开发。尖端**已含** `3.1.6.4`（覆盖合入）；**保留** `3.1.6.4` |
| **合入要点** | |
...
| **建议** | 日常开发用此分支；发版跟 `3.1.6` |

### `3.1.6.4`（招式流程线，已并入 3.1.6.3）

| 项 | 说明 |
|---|---|
| **定位** | 招式库 / 流程改造与方案事件半成品线 |
| **状态** | 尖端已由 `3.1.6.3` **覆盖合入**；**分支保留**对照 / 回溯，不再作为发布线 |
| **package** | `3.1.6.3`（历史提交时随开发线） |
| **要点** | 见上一节「招式流程」与「方案 / 事件」；专题说明见 `feature/skill-flow-redesign` |
...
| 3.1.6.3 | handoff1 + 招式流程 / 方案事件 + 计算器构图与切换性能 |
