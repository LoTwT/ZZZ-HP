...
| | · **计算器 UI**：伤害页构图对齐参考布局（计算方式 → 编队 → 面板/额外Buff/敌方 → 招式流程 → 伤害结果）；白天模式对齐危局纸色底+点阵、侧栏保持深色菜单 |
| **刻意保留（不覆盖）** | |
...
| **要点** | 复用 `crisisScoreHpTable.ts` 累计拐点；正常/困难两套输入互相独立；已打血量%与分数双向线性插值；总血量/怪物库优先，改占比时已打血量跟着变、改已打血量时占比跟着变。页面展示插值区间与算式便于对数。可记录最多 10 条当前换算结果（空表默认 5 行；已打血量 / 分数占比 / 伤害占比 / 具体分数；较上/下一条以已打血量为基准，数字完整显示；可单行删除） |
| **状态** | 本地开发，未推远程 |
